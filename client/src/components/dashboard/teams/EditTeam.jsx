import { useEffect, useState } from "react";
import { useDispatch, useSelector } from "react-redux";
import { useNavigate, useParams } from "react-router-dom";
import Formfields from "../../common/Formfields";
import { getTeamById, updateTeam } from "../../../redux/slice/teamsSlice";

function EditTeam() {
  const [formData, setFormData] = useState({});
  const [logoPreview, setLogoPreview] = useState(null);
  const dispatch = useDispatch();
  const navigate = useNavigate();
  const { auctionId, teamId } = useParams();
  const { selectedTeam, loading } = useSelector((state) => state.teams);

  useEffect(() => {
    dispatch(getTeamById({ auctionId, teamId }));
  }, [auctionId]);

  useEffect(() => {
    if (selectedTeam) {
      setFormData({
        logo: selectedTeam.logo,
        name: selectedTeam.name,
      });
    }
  }, [selectedTeam]);

  const handleChange = (e) => {
    const { name, value, type, files } = e.target;
    if (type === "file" && name === "logo") {
      const file = files[0];
      setFormData((prev) => ({
        ...prev,
        [name]: file,
      }));
      if (file) {
        setLogoPreview(URL.createObjectURL(file));
      }
    } else {
      setFormData((prev) => ({
        ...prev,
        [name]: value,
      }));
    }
  };

  const handleSubmit = async (e) => {
    e.preventDefault();
    const data = new FormData();

    Object.keys(formData).forEach((key) => {
      data.append(key, formData[key]);
    });

    if (selectedTeam) {
      data.append("auctionId", selectedTeam.auction);
      data.append("teamId", selectedTeam._id);
    }

    const res = await dispatch(updateTeam(data));
    if (res.payload?.success) {
      navigate(-1);
    }
  };

  return (
    <div className="bg-white p-4 rounded shadow">
      <h2 className="text-2xl font-medium">Edit Auction</h2>
      <div className="mt-6 xl:w-[70%]">
        <form onSubmit={handleSubmit}>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <img
                src={logoPreview || selectedTeam?.logo}
                alt="AuctionLogo"
                className="h-24 w-32 object-contain"
              />
            </div>

            <Formfields
              label="Team Logo"
              name="logo"
              type="file"
              onChange={handleChange}
            />
            <Formfields
              label="Team Name"
              name="name"
              type="text"
              value={formData.name ?? ""}
              onChange={handleChange}
              placeholder="Enter team name"
            />
          </div>

          <button
            type="submit"
            className="mt-4 px-4 bg-blue-600 text-white py-2 rounded hover:bg-blue-800 transition"
            disabled={loading}
          >
            {loading ? "Updating..." : "Update Auction"}
          </button>
          <button
            className="ml-4 py-2 bg-red-500 hover:bg-red-700 rounded px-6 text-white transition"
            onClick={() => navigate(-1)}
            type="button"
          >
            Cancel
          </button>
        </form>
      </div>
    </div>
  );
}

export default EditTeam;
